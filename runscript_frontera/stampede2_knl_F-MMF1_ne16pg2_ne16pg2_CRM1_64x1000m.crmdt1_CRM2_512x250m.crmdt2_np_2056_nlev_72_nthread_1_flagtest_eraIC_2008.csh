...
set job_name       = smoketest_fast_IC_v2_F-MMF1_hflag_eraIC_openmp_${machine}_${resolution}_CRM1_${crm_nx}x_${crm_nz}z${crm_dx}m.${crm_dt}s_crm_nx_rad_${crm_nx_rad}_CRM2_${crm_nx2}x_${crm_nz2}z${crm_dx2}m.${crm_dt2}s_crm_nx_rad2_${crm_nx_rad2}_np_${np}_nlev_${nlev}_nthread_${OMP_NUM_THREADS}
...
set processor_config = customknl        # PE count: S (39 nodes), L (285 nodes)
...
### DIRECTORIES
set code_root_dir               = /home1/07088/tg863871/repositories
set e3sm_simulations_dir        = /scratch1/07088/tg863871
...
#atm_output_freq (the namelist variable is nhqfrq) : The frequency with which the atmosphere writes its output.
...
if ( `lowercase $fetch_code` == true ) then
  ...
  mkdir -p $code_root_dir/$tag_name
  cd $code_root_dir/$tag_name
  # This will clone repository, with all code, in directory $tag_name
  git init
  git remote add ${e3sm_tag} https://github.com/E3SM-Project/E3SM.git
  git fetch ${e3sm_tag}
  #git cherry-pick fa04d06e6c594673fb89970884f9bfc059f034f2
...
if ( `lowercase $processor_config` == '1' ) then
 ...
 foreach nthrds_name ( NTHRDS_ATM  NTHRDS_LND  NTHRDS_ICE  NTHRDS_OCN  NTHRDS_CPL  NTHRDS_GLC  NTHRDS_ROF  NTHRDS_WAV )
  ...
  ${xmlchange_exe} MAX_TASKS_PER_NODE="56"
 # ${xmlchange_exe} PES_PER_NODE="256"

  ${xmlchange_exe} NTASKS_ATM="$natm"
  ${xmlchange_exe} ROOTPE_ATM="0"

  ${xmlchange_exe} NTASKS_LND="$nother"
  ${xmlchange_exe} ROOTPE_LND="0"

  ${xmlchange_exe} NTASKS_ICE="$nother"
  ${xmlchange_exe} ROOTPE_ICE="0"

  ${xmlchange_exe} NTASKS_OCN="$nother"
  ${xmlchange_exe} ROOTPE_OCN="0"

  ${xmlchange_exe} NTASKS_CPL="$natm"
  ${xmlchange_exe} ROOTPE_CPL="0"

  ${xmlchange_exe} NTASKS_GLC="1"
  ${xmlchange_exe} ROOTPE_GLC="0"

  ${xmlchange_exe} NTASKS_ROF="1"
  ${xmlchange_exe} ROOTPE_ROF="0"

  ${xmlchange_exe} NTASKS_WAV="1"
  ${xmlchange_exe} ROOTPE_WAV="0"

  ${xmlchange_exe} NTHRDS_ATM="$ThreadCount_atm"
...
cat <<EOF >> user_nl_cam
 ...
 aerodep_flx_datapath = '/scratch1/07088/tg863871/inputdata/atm/cam/chem/trop_mam/aero'
 aerodep_flx_file = 'mam4_0.9x1.2_L72_2000clim_c170323.nc'
 aerodep_flx_cycle_yr = 01
 prescribed_aero_type           = 'CYCLICAL'
 prescribed_aero_datapath='/scratch1/07088/tg863871/inputdata/atm/cam/chem/trop_mam/aero'
 prescribed_aero_file = 'mam4_0.9x1.2_L72_2000clim_c170323.nc'
 heavy_load_file = '/scratch1/07088/tg863871/inputdata/Liran_Flag/ne16pg2_Flag.nc'
...
else if ( $machine == stampede2-knl || $machine == stampede2 ) then
    e3sm_print "Setup stampede2-knl-liran machine"
    set batch_options = "--job-name=${job_name} --output=batch_output/${case_name}.o%j -n ${nnum}"
