...
contains
  ...
  subroutine buoyancy(ncrms)
    ...
    real(crm_rknd) betu, betd, dwdt_buoy
      ...
      do j=1,ny
        do i=1,nx
          do icrm=1,ncrms
            ...
            dwdt(icrm,i,j,k,na)=dwdt(icrm,i,j,k,na) +  dwdt_buoy

            crm_buoya(k,icrm) = crm_buoya(k,icrm) + w(icrm,i,j,k)*dwdt_buoy

            buoya_tot(k,icrm) = buoya_tot(k,icrm) + dtn*w(icrm,i,j,k)*dwdt_buoy
